...
The density figure should be read horizontally within each dataset rather than by comparing raw error between indoor and outdoor panels. The left plots are error metrics where lower is better; the right plots are threshold accuracies where higher is better. The sample-count axis is logarithmic: moving equally far right means multiplying the number of measurements. The RGBd error curve drops quickly at low sample counts and then flattens even as the absolute number of additional measurements grows. That shape is the central result: a small number of metric anchors resolves a large scale ambiguity, while later samples mostly refine already-supported surfaces.
