...
> InternVL3.5 treats capability and serving cost as one design problem. Cascade RL first uses offline mixed preference optimization to produce reliable rollouts, then online GSPO refines the model's output distribution. The Flash variants add a visual resolution router that compresses easy image patches more aggressively, while Decoupled Vision-Language Deployment runs the vision and language subsystems on separate servers. At 8B, the reasoning average rises from InternVL3’s 44.3 to InternVL3.5’s 60.3, a 16.0-point cross-generation gain. Within InternVL3.5, Cascade RL moves the SFT checkpoint from 53.6 to 60.3. The separate 4.05× serving gain comes from a 38B deployment at 896 resolution with DvD and ViR, not that reasoning comparison.

## Core Insights

### Offline training gives online RL a stronger starting point
...
*Fig 1: InternVL3.5 uses roughly 250B pre-training tokens, 130B SFT tokens, and a 270K-sample Cascade RL stage; InternVL3.5-Flash adds roughly 30B-token visual consistency and router training. | source: [InternVL3.5, Figure 3](https://arxiv.org/abs/2508.18265)*
...
### A router allocates visual tokens where detail matters
...
*Fig 2: InternVL3.5 scores are averaged across general multimodal, reasoning, text, and agentic benchmarks; hatched bars are closed-source models. This figure is a breadth comparison, not a single-task causal ablation. | source: [InternVL3.5, Figure 1](https://arxiv.org/abs/2508.18265)*
...
### Separate serving removes interference between vision and decoding
