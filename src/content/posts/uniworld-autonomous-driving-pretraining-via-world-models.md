...
**Summary:** UniWorld pretrains a camera encoder and BEV representation by predicting geometric occupancy across time. Multi-frame LiDAR is fused offline to generate 4D occupancy supervision; a decoder reconstructs that target during pretraining and is discarded for downstream camera-only fine-tuning.
...
**Context:** UniM²AE reconstructs masked synchronized sensors; UniWorld predicts 4D occupancy; ViDAR predicts future point clouds from historical images.

**Takeaway:** Privileged LiDAR can supervise a reusable camera world representation, but its generated geometry must be treated as a versioned, fallible label source.
