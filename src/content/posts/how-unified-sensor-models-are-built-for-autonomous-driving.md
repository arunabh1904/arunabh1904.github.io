...
The perception stack should preserve those measurement differences before it combines them. Camera encoders need to retain texture, boundaries, and small objects across several image scales. LiDAR encoders need to process occupied 3D space without paying for the much larger empty volume. Radar encoders need to keep Doppler, return strength, time, and uncertainty attached to each measurement. Fusion only becomes meaningful after calibration and synchronization map those features into the same vehicle coordinate frame.
...
_The runtime path moves from sensor-specific features to a calibrated BEV field or query set, then carries that state into task heads. The lower path shows supervision that may exist during training without becoming a deployed sensor input._
...
Camera encoders are strongest where appearance matters: class identity, lane paint, signs, lights, and object boundaries. Their main 3D weakness is not a lack of semantics but depth ambiguity. Increasing input resolution or backbone capacity can improve recognition without fixing that ambiguity. This is why the next architectural decision—how image features are lifted into metric space—often matters as much as the image backbone itself.
...
## From camera features to 3D

A pixel fixes a ray through the camera center, not a distance along that ray. Camera-based 3D perception must decide where along the ray to place its feature. The main methods differ in which object makes that decision: an image pixel, a metric voxel, an object query, or a BEV cell.

[Lift, Splat, Shoot](/paper%20shorts/2020/08/13/lift-splat-shoot-encoding-images-from-arbitrary-camera-rigs.html) predicts a categorical depth distribution for each image location, copies the image feature along the corresponding depth bins, and pools those lifted features into BEV. [BEVDepth](/paper%20shorts/2022/06/21/bevdepth-acquisition-of-reliable-depth-for-multiview-3d-detection.html) adds projected LiDAR supervision to make that depth distribution more reliable. These forward-projection methods preserve dense image evidence, which is useful for occupancy and maps, but a depth error writes the feature into the wrong metric cell.
...
Query methods make the metric support selective. [DETR3D](/paper%20shorts/2021/10/14/detr3d-multiview-images-via-3d-to-2d-queries.html) assigns each object query a reference point $(x,y,z)$ and samples image evidence around its projections. BEVFormer assigns each BEV cell an $(x,y)$ position and samples several heights along the vertical pillar. [FB-BEV](https://openaccess.thecvf.com/content/ICCV2023/html/Li_FB-BEV_BEV_Representation_from_Forward-Backward_View_Transformations_ICCV_2023_paper.html) combines forward depth lifting with backward BEV-to-image sampling, reflecting the complementary errors of the two directions.
...
No row dominates every task. Dense scene outputs need broad spatial support; sparse actor outputs can spend their budget on hypotheses. The relevant question is which evidence the representation can no longer recover after the 2D-to-3D step.

## Fusion and sensor failure

Once every modality has metric support, fusion can happen at points, object queries, or dense BEV cells. [PointPainting](/paper%20shorts/2019/11/22/pointpainting-sequential-fusion-for-3d-object-detection.html) attaches camera class scores to LiDAR points before voxelization. This is cheap and geometrically direct, but image evidence survives only where LiDAR produced a return. [FUTR3D](/paper%20shorts/2022/03/20/futr3d-unified-sensor-fusion-framework-for-3d-detection.html) samples camera, LiDAR, and radar features around the same 3D object reference point. It matches an object-centric output but does not retain a complete background field. [BEVFusion](/paper%20shorts/2022/05/26/bevfusion-multi-task-multi-sensor-unified-bev.html) builds camera and LiDAR BEV features independently and fuses the aligned maps, which supports both object detection and dense map segmentation.
...
BEV is the most general fusion interface when one shared representation must support detection, occupancy, and mapping, but it is not automatically the best detector. Query fusion avoids allocating a full grid and keeps object identity explicit. Proposal-guided methods such as [TransFusion](/paper%20shorts/2022/03/22/transfusion-robust-lidar-camera-fusion-with-transformers.html) can also soften image-LiDAR correspondence: a LiDAR proposal attends over a neighborhood of image evidence instead of trusting one calibrated pixel. [DeepInteraction](/paper%20shorts/2022/08/23/deepinteraction-3d-object-detection-via-modality-interaction.html) preserves separate streams while they update one another, and [UniTR](/paper%20shorts/2023/08/15/unitr-unified-efficient-multimodal-transformer-for-bev.html) applies shared transformer blocks after modality-specific tokenization. There is no useful sensor-fusion “state of the art” without fixing the task, sensor set, range, latency, and failure suite. The durable design goals are to preserve sensor provenance, tolerate small alignment errors, and spend compute on the required outputs.
...
## Time, memory, and sparse computation
...
[BEVDet4D](/paper%20shorts/2022/03/31/bevdet4d-temporal-cues-in-multicamera-3d-detection.html) is the useful dense baseline: warp the previous BEV feature into the current ego frame, concatenate it with the current feature, and let a BEV encoder learn displacement cues. BEVFormer uses temporal attention to update a recurrent BEV field. Dense state retains roads, free space, and weak background evidence that may later support a new detection, but its memory and warp cost scale with BEV area and history.
...
Sparse temporal models carry hypotheses instead of a full field. [Sparse4D v2](/paper%20shorts/2023/05/23/sparse4dv2-recurrent-temporal-fusion-with-sparse-model.html) transforms prior instances into the current frame and combines them with fresh anchors. [StreamPETR](/paper%20shorts/2023/03/21/streampetr-object-centric-temporal-modeling-for-multiview-3d-detection.html) retains a FIFO queue of top foreground queries, conditions them on ego pose, elapsed time, and velocity, and discards background queries. This bounds temporal state, but query birth, duplicate removal, aging, and stale false positives become part of the model rather than incidental bookkeeping.

[SOLOFusion](/paper%20shorts/2022/10/05/solofusion-temporal-multiview-3d-object-detection.html) exposes another temporal trade-off. It uses a short high-resolution history for fine stereo correspondence and a longer low-resolution BEV history for depth and velocity. [SparseBEV](/paper%20shorts/2023/08/18/sparsebev-high-performance-sparse-3d-object-detection.html) keeps sparse object support but retrieves from several stored frames, so its cost still grows with history length. “Sparse temporal” can therefore mean compressing history into recurrent state or retaining history and reading only selected locations; the two designs have different memory, latency, and error-accumulation behavior.
...
## Training the shared representation
