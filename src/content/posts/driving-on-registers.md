...
The first figure is a diagnostic of the compression itself. Darker entries mean lower cosine similarity: front-camera registers separate into distinct scene roles, while back-camera registers tend to collapse. That pattern is a useful warning against counting tokens alone; the registers have to specialize to preserve planning evidence. The second figure shows the proposal-scoring contract: learned queries generate multiple trajectories, each candidate is re-embedded for scoring, and the final choice maximizes predicted safety, comfort, and efficiency components.


![Figure 4 from Driving on Registers](/assets/images/driving-on-registers-source-figure-4.webp)
*Fig 1: Cosine similarity between scene tokens. Darker indicates lower cosine similarity. | source: [Driving on Registers](https://arxiv.org/abs/2601.05083)*

![Figure 1 from Driving on Registers](/assets/images/driving-on-registers-source-figure-1.webp)
*Fig 2: DrivoR architecture. The proposed architecture is composed of three transformer blocks: one encoder (perception) and two decoders (trajectory and scoring). | source: [Driving on Registers](https://arxiv.org/abs/2601.05083)*
