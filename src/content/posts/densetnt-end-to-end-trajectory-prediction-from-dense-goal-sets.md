...
*Figure 2 shows the dense-goal pipeline: vectorized context features become a goal heatmap, a learned goal-set predictor selects goals, and a trajectory decoder completes the futures. From the [DenseTNT paper](https://arxiv.org/abs/2108.09640). source: [DenseTNT paper](https://arxiv.org/abs/2108.09640)*
...
*Figure 3 Two-stage training of goal set predictor. In the first stage, we use the ground truth goals to train all the modules except for the goal set predictor. In the second stage, we only train the goal set predictor, using pseudo-labels generated by the optimization algorithm. source: [DenseTNT: End-to-End Trajectory Prediction from Dense Goal Sets](https://arxiv.org/abs/2108.09640)*
