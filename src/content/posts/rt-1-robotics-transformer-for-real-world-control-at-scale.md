...
The speed experiment also clarifies what the model is not doing. Autoregressively conditioning action tokens increases inference to 36 ms and reduces seen, unseen, and distractor scores to 85/71/67, although background robustness improves from 59 to 65, so the released design predicts the action dimensions without feeding each predicted action back into the next step. A continuous Gaussian action head drops seen-task success from 97% to 68% and unseen-task success from 76% to 43%; the paper attributes the gap to the inability of a single Gaussian mode to represent the varied action distributions in the demonstrations.

### What the data experiments isolate

On the main evaluation, RT-1 reaches 97% on seen instructions, 76% on unseen instructions, 83% under distractors, and 59% under new backgrounds. The protocol includes over 200 seen tasks, 21 unseen instructions, 30 distractor tasks, and 22 background tasks, with variations in object placement, time of day, and robot position. Gato, BC-Z, and BC-Z XL are retrained on the same RT-1 dataset, so this comparison controls robot data rather than comparing their original published checkpoints. A more realistic three-level kitchen evaluation reports 70% overall: 88% for the new layout and lighting condition, 75% after adding unseen distractors, and 50% after adding more radical task, object, or location changes.
...
Within SayCan, the evaluation uses 15 instructions averaging 9.6 steps. Planning succeeds 87% and execution 67% in each of Kitchen1 and Kitchen2. A separate supplementary video demonstrates up to 50 steps; that longer demonstration is not the protocol behind the table. The stable score across kitchens is a strong test of short-horizon skill robustness, but it does not mean RT-1 learned long-horizon planning itself: SayCan proposes the sequence and RT-1 executes each instruction.
