...
The Modality-Agnostic Feature Sampler (MAFS) implements that question differently for each representation. It projects the reference point into camera feature maps, samples multi-scale BEV features for LiDAR, and samples a radar feature map built from radar pillars. For LiDAR and radar, predicted offsets and attention weights let a query look around its projected BEV point instead of accepting one brittle correspondence. Camera sampling is different: intrinsics and extrinsics fix the projected image coordinates, and the query predicts scalar weights over bilinear samples from those locations. The samples are fused by an MLP with positional encoding and returned to the query. A transformer decoder then predicts the box center, dimensions, yaw, velocity, and class. Iterative refinement feeds the new center back as the next reference point.

Each encoder keeps its native coordinates; the sampler and decoder make the object hypothesis the shared interface, with DETR3D and Object DGCNN as single-modality special cases.
...
On the validation split, the camera-only FUTR3D model reaches 10.4 mAP beyond 30 m. Four-beam LiDAR alone reaches 16.1, while the combination reaches 27.4. Cameras supply dense appearance for distant or small objects, while even sparse LiDAR supplies metric placement. With cameras plus 4-beam LiDAR over all ranges, FUTR3D reaches 58.0 mAP, compared with 56.6 for the cited 32-beam CenterPoint result. This cross-method comparison is evidence for the low-cost configuration, not a controlled replacement claim.
...
The query interface also handles radar. In the camera-radar experiment, adding radar to the camera model raises mAP from 34.6 to 39.9 and NDS from 42.5 to 51.1; mAVE, shown in the table's ×100 reporting scale, falls from 84.2 to 41.3, roughly a 51% reduction. Radar adds sparse velocity and localization evidence that cameras must infer.
