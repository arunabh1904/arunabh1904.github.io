...
Read the first figure as a comparison of where multimodality enters the system: regression has one path, a fixed vocabulary has many but only where its anchors reach, vanilla diffusion has a flexible distribution but a long sampling chain, and DiffusionDrive combines an anchored prior with short refinement. The qualitative panel makes the trade-off visible by separating the top-ranked path from the other plausible candidates.
...
*Fig 3: The paradigms progress from single-mode regression to discrete vocabulary sampling, full-step diffusion, and DiffusionDrive’s truncated diffusion policy. | source: [DiffusionDrive: Truncated Diffusion Model for End-to-End Autonomous Driving](https://arxiv.org/abs/2411.15139)*
