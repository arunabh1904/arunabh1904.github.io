...
**Summary:** SENNA uses a hybrid architecture: a vision-language model produces a high-level textual plan, while an end-to-end driving module converts that plan and the sensor input into a precise trajectory. The design avoids asking the LVLM to output exact steering-level control directly.
...
**Takeaway:** The most useful VLM in a driving stack may be the one that thinks out loud at the right abstraction level.
