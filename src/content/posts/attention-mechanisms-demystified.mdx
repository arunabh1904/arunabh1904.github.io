...
*Figure 3 shows four soft alignments learned without word-level alignment labels. The diagonal structure tracks mostly monotonic translation, while off-diagonal bands capture phrases whose word order changes between English and French. source: [Neural Machine Translation by Jointly Learning to Align and Translate](https://arxiv.org/abs/1409.0473)*
...
*Figure 2 puts the operations in dependency order: compare queries with keys, scale and normalize the scores, then mix values. Its multi-head diagram repeats that operation across learned projections before concatenating the results. source: [Attention Is All You Need](https://arxiv.org/abs/1706.03762)*
...
Consider the sentence with bidirectional self-attention, where the whole sentence is visible:
...
That example explains a possible routing pattern, not a measured attention trace. A causal decoder updating `it` cannot yet read `tired`, which occurs later in the sentence.
...
The complete causal multi-head block for a full input sequence is therefore short. The projections produce `[batch, time, 3 * d_model]`; reshaping exposes the head axis expected by SDPA; `is_causal=True` applies the autoregressive mask without allocating a dense mask in Python.
...
This block recomputes keys and values for the supplied sequence. It is a training or prefill example; the cached decode helper later in the post shows how to process a new token without rebuilding the prefix.
...
<div className="architecture-comparison blog-frame-explainer" data-blog-frame-explainer="blog-attention-memory.gif"><div className="blog-frame-explainer__viewport"><a href="/assets/images/blog-explainer-frames/blog-attention-memory/frame-01.webp"><img src="/assets/images/blog-explainer-frames/blog-attention-memory/frame-01.webp" alt="Manual explainer comparing the memory retained by multi-head attention, grouped-query attention, multi-head latent attention, and Gated DeltaNet" /></a></div></div>
...
*Figure 2 places the three cache layouts on one spectrum. MHA keeps one K/V pair per query head, GQA shares K/V within groups, and MQA shares one K/V pair across all query heads. source: [GQA: Training Generalized Multi-Query Transformer Models from Multi-Head Checkpoints](https://arxiv.org/abs/2305.13245)*
...
PyTorch's SDPA accepts different query-head and KV-head counts through `enable_gqa=True`. The following implementation keeps the K/V tensors at `n_kv_heads`; it leaves head sharing to the backend. An optimized GQA kernel can reuse those heads directly; a fallback may still expand them internally, so the API flag alone is not a speed guarantee.
...
For `n_query_heads=32` and `n_kv_heads=8`, four query heads share each stored K/V head, so the K/V portion of the cache is one quarter of the corresponding 32-head MHA cache. The decode helper also preallocates and updates that cache rather than calling `torch.cat` every step, which would repeatedly copy the growing prefix. Native GQA support is still experimental, so confirm support on the target device and PyTorch version.

The `is_causal=False` in the single-token helper is deliberate. Every key in the sliced cache is already eligible. With a one-row query and a longer key sequence, SDPA's upper-left causal mask would expose only the first key, not the entire prefix. Decoding several new tokens together needs a different mask that also orders those new tokens.
...
then writes only the prediction error. Here $S$ has shape $d_k\times d_v$, so the outer product below updates the same key-by-value memory:
...
Before implementing the delta rule, start with the simpler additive linear-attention recurrence above. Its state update is easiest to see one decode step at a time. With the positive feature map $\phi(x)=\operatorname{ELU}(x)+1$, the following code updates $S_t$ and $z_t$ without retaining a token-length dimension. It is vectorized over batches and heads, and `torch.compile` can fuse its pointwise and contraction graph on supported backends.
...
*Figure 1 shows two hybrid stacks and the Gated DeltaNet block. H1 alternates Gated DeltaNet with sliding-window attention; H2 additionally uses Mamba2. The block produces $q$, $k$, $v$, a decay gate $\alpha$, and a write gate $\beta$ before applying the gated delta rule. source: [Gated Delta Networks: Improving Mamba2 with Delta Rule](https://arxiv.org/abs/2412.06464)*
...
The design principle is more important than any one ratio. Recurrent or linear layers carry a cheap running summary, while occasional full-attention layers retain a separate token-indexed path for retrieval. Those layers can revisit states within their retained cache window; they cannot reconstruct evidence that every earlier representation already discarded. Gated attention modifies those retained full-attention blocks: an output gate controls how much retrieved information enters the residual stream, but the block still performs scaled dot-product attention.
