...
The ablation separates complementary geometric cues from fusion capacity. Sparse Depth Guidance alone adds 0.8 mAP/0.4 NDS, LiDAR Occupancy Guidance alone adds 1.0/0.6, and using both reaches 1.4/0.8 rather than simply duplicating one signal. Local-global adaptive fusion then improves by roughly 0.7 mAP over plain addition or concatenation, while the two-frame temporal path contributes only about 0.3 mAP/0.1 NDS. The ordering suggests that geometry guidance does most of the repair and temporal context is a smaller refinement; a deployment decision should therefore measure whether the extra frame and LiDAR dependence justify that last increment.
...
*Fig 1: LiDAR guides the camera branch twice—through sparse depth and occupancy—before adaptive local-global fusion and short temporal aggregation. | source: [GAFusion](https://openaccess.thecvf.com/content/CVPR2024/html/Li_GAFusion_Adaptive_Fusing_LiDAR_and_Camera_with_Multiple_Guidance_for_CVPR_2024_paper.html)*
