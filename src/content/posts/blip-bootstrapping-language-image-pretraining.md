...
*Fig 1: Pre-training model architecture and objectives of BLIP (same parameters have the same color). The multimodal mixture of encoder-decoder can operate as a unimodal encoder for image-text contrastive learning, an image-grounded text encoder for image-text matching, or an image-grounded text decoder for caption generation. | source: [BLIP: Bootstrapping Language-Image Pre-training for Unified Vision-Language Understanding and Generation, Figure 2](https://arxiv.org/abs/2201.12086)*
...
*Fig 2: Examples of the web text $T_w$ and synthetic text $T_s$. Green texts are accepted by the filter, whereas red texts are rejected. | source: [BLIP: Bootstrapping Language-Image Pre-training for Unified Vision-Language Understanding and Generation, Figure 4](https://arxiv.org/abs/2201.12086)*

The data problem is noisy alt text. BLIP pre-trains on about 14M images from COCO, Visual Genome, Conceptual Captions 3M and 12M, and SBU; an expanded experiment adds 115M images from LAION for a 129M-image mixture. The authors first fine-tune two copies of MED on COCO. The captioner is the image-grounded decoder and writes one synthetic caption for each web image. The filter is the image-grounded encoder and uses ITC and ITM to decide whether either the original web text or the synthetic caption matches the image. Only the accepted pairs, together with human-annotated pairs, form the dataset for a fresh BLIP pre-training run.
...
The ablations explain why generation diversity and judge independence matter. Nucleus sampling with cumulative probability threshold $p=0.9$ produces a 25% noise ratio, compared with 19% for beam search, yet its cleaned data yields COCO retrieval TR@1/IR@1 of 80.6/63.1 versus 79.6/61.9 for beam search. The authors attribute the gain to more varied captions that add information beyond common phrases. When the captioner and filter share parameters, the noise ratio falls to 8% and the downstream scores are lower than with decoupled copies; the paper connects this to confirmation bias, because a judge too close to its generator is less likely to reject the generator’s errors.

The improvement is also a data-quality effect rather than just extra steps. When the original web text is replicated to match the bootstrapped dataset’s samples per epoch, the noisy control reaches COCO retrieval 78.3/60.5 while CapFilt reaches 80.6/63.1. Continuing the old pre-trained model on the bootstrapped data is slightly weaker than training a new model: NoCaps CIDEr is 104.5 versus 105.1, and COCO caption CIDEr is 129.9 versus 129.7 in the reported comparison. The cleaner distribution changes what the model learns, so simply showing it more often is not an equivalent intervention.
