...
The planner first predicts one of four meta-decisions—keep stationary, keep speed, accelerate, or decelerate—then autoregressively emits floating-point waypoints. WOMD uses a five-second horizon divided into two 2.5-second stages; nuScenes uses a three-second horizon in one stage. The high-level command is generated from heuristics and the future trajectory, while the waypoint target remains the actual self-supervised signal. That makes the hierarchy useful as a planning scaffold, but it is not free of supervision or leakage: the model still learns from future trajectories and the paper's command construction.

The second figure makes the training contract concrete. It shows the prompt, historical ego state, multi-view context, and target response as text, rather than implying that the MLLM discovers a driving objective without labels. At test time, the authors also draw multiple nucleus samples and average their waypoints. Sixteen samples give 0.693/0.928 ADE/bADE on WOMD, better than greedy decoding (0.728/0.986) and four-sample beam search (0.739/0.997); weighted averaging is worse (0.747/1.005). The averaging is a practical way to temper the model's overconfidence on straight and stop cases.
...
*Fig 2: The source prompt supplies multi-view history and ego state, and the target response contains the maneuver decision and future trajectory. | source: [S4-Driver: Scalable Self-Supervised Driving Multimodal Large Language Model with Spatio-Temporal Visual Representation, Figure 9](https://arxiv.org/abs/2505.24139)*
...
- “Self-supervised” here means no intermediate object or map labels; future trajectories and heuristic maneuver targets still define the learning problem, and the paper does not report a closed-loop safety score.
