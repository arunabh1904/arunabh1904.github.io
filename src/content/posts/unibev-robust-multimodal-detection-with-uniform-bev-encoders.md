...
**Context:** UniBEV turns the missing-modality problem into a training distribution and fusion-normalization problem, complementing MetaBEV's expert routing and later reliability-aware work.

**Takeaway:** A single fallback-capable model works only when missing-sensor modes are trained explicitly and fusion is normalized over the evidence that remains.
