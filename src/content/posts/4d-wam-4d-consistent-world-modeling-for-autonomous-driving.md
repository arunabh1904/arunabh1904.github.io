...
*Fig 1: A frozen geometric foundation model supervises predicted future frames during training; the deployed WAM keeps the original inference path. | source: [4D-WAM](https://arxiv.org/abs/2608.10107)*
...
*Fig 2: Video and action branches settle on a driving decision while diffusion noise is still high; action-to-final MSE then plateaus, separating an early decision region from later visual refinement. | source: [4D-WAM: 4D Consistent World Modeling for Autonomous Driving](https://arxiv.org/abs/2608.10107)*
