...
*Fig 1: Provides the key reduction: every denoising chain becomes an inner MDP with tractable Gaussian transitions, so environment reward can train the diffusion policy with ordinary policy gradients. | source: [DPPO](https://arxiv.org/abs/2409.00588)*
...
*Fig 2: Comparing to other policy parameterizations in the more challenging Square and Transport tasks from Robomimic, with state (left) or pixel (right) observation. Results are averaged over three seeds. | source: [DPPO: Diffusion Policy Policy Optimization](https://arxiv.org/abs/2409.00588)*
...
*Fig 3: Long-horizon robot manipulations tasks including (left) the bimanual Transport from Robomimic and (right) Furniture - Bench tasks (full rollouts visualized in Fig. A12 ). | source: [DPPO: Diffusion Policy Policy Optimization](https://arxiv.org/abs/2409.00588)*
