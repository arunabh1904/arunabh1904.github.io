...
### A discrete action vocabulary preserves physical meaning
...
### Reward decides when reasoning earns its cost
...
The tokenization ablation explains why the interface matters. At $K=2048$, K-disk reconstruction reaches ADE 0.0182 m and FDE 0.0203 m with 99.42% movement coverage and 100% codebook usage; at $K=4096$, reconstruction improves but usage falls to 91.46%. In the mixed nuPlan/nuScenes tokenization experiment, Table 5 combines NAVSIM PDMS with trajectory and runtime metrics: physical action tokens score PDMS 80.54, average L2 0.70 m, collision 0.31%, and runtime 3.95 s, versus 71.31, 0.89 m, 0.36%, and 7.65 s for text waypoints. The paper’s limitation remains material: even its near-real-time mode runs at about 1 Hz and is GPU-dependent.
...
- RFT’s strongest claim is selective reasoning: it improves one-shot NAVSIM behavior while lowering average cost by choosing shorter outputs more often; the measured fast and slow modes still average 1.072 and 10.518 seconds.
