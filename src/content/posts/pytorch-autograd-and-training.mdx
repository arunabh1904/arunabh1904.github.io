...
[![Animation showing the forward pass recording an autograd graph and backward traversing it toward leaf parameters](/assets/images/pytorch-autograd-forward-backward.gif)](/assets/images/pytorch-autograd-forward-backward.gif)

Forward creates the nodes and saves only the values their backward rules require. Backward then starts at the scalar loss with an upstream gradient of one, composes local vector-Jacobian products in reverse, and accumulates the results on the leaf parameters.
...
[![Animation showing three microbatch gradients accumulating before one optimizer step and gradient reset](/assets/images/pytorch-gradient-accumulation.gif)](/assets/images/pytorch-gradient-accumulation.gif)

Each backward adds a scaled contribution to the same `.grad` buffer. The optimizer reads the completed sum once, updates the parameter, and only then does `zero_grad(set_to_none=True)` remove the buffer for the next accumulation window.
