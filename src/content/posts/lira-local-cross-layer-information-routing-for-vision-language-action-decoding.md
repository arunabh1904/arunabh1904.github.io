...
![LIRA local cross-layer routing between VLM features and action decoder blocks](/assets/images/lira-overview-paper-figure.png)
*Each Parallel Fusion Block reads a depth-aligned local window of intermediate VLM features before action prediction. source: [LIRA](https://arxiv.org/abs/2608.07596)*
...
*Figure 2 Overview of LIRA . The Prismatic-style VLM processes visual-language task tokens together with learnable LIRA Query tokens, while Action Query tokens initialize an action decoder composed of Parallel Fusion Blocks (PFBs). Task-token features remain aligned with the corresponding VLM depth, while each PFB receives LIRA Query features aggregated from a centered local window of adjacent VLM layers. Each PFB uses Action Query features to fuse the two conditioning sources before action prediction. source: [LIRA: Local Cross-Layer Information Routing for Vision-Language-Action Decoding](https://arxiv.org/abs/2608.07596)*
...
*Figure 1 Zero-shot transfer to LIBERO-Plus. Left: representative LIRA rollouts for the same language instruction under Background, Layout, and Light perturbations; all three are completed successfully without LIBERO-Plus fine-tuning. Right: overall task-level success over all 10,030 LIBERO-Plus tasks. LIRA achieves 78.0%, outperforming OpenVLA-OFT and VLA-Adapter by 8.4 and 18.9 percentage points, respectively. source: [LIRA: Local Cross-Layer Information Routing for Vision-Language-Action Decoding](https://arxiv.org/abs/2608.07596)*
