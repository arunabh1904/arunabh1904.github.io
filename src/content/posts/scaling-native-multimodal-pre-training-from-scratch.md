...
*Sweeping the mixture ratio traces different language–multimodal trade-offs at fixed total compute; the fitted allocation changes with the target mixture. org/abs/2607.22043). source: [paper](https://arxiv.org/abs/2607.22043)*
...
*Figure 1 IsoFLOP curves (language objective). For a range of model sizes, we adjust the number of training tokens to maintain a constant final FLOPs, setting the cosine cycle length to match this target compute budget. The distinct valley in the loss curve demonstrates that an optimal model size exists for any given FLOP budget. Based on the locations of these minima, we project the compute-optimal model size and token count for larger scales. source: [Scaling Native Multimodal Pre-Training From Scratch](https://arxiv.org/abs/2607.22043)*
