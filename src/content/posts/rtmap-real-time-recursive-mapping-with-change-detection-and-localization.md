...
*Fig 1: Shows RTMap's loop: encode sensors and the crowdsourced HD map, match current and prior elements, estimate pose, detect changes, and fuse local maps offline. | source: [RTMap paper](https://arxiv.org/abs/2507.00980)*
...
*Fig 2: On TbV, state optimization and explicit map-change handling reduce trajectory deviation from ground truth, showing that filtering stale map elements improves localization. | source: [RTMap: Real-Time Recursive Mapping with Change Detection and Localization](https://arxiv.org/abs/2507.00980)*
