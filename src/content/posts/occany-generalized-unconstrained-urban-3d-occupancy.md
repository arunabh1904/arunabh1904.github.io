...
*OccAny is a generalized 3D occupancy model that is trained once and can operate on out-of-domain sequential, monocular, or surround-view urban images. source: [OccAny: Generalized Unconstrained Urban 3D Occupancy](https://arxiv.org/abs/2603.23502)*
...
*Figure 2 OccAny Training is done in two stages: (i) 3D Reconstruction infers 3D scene using reconstruction frames and (ii) Novel-View Rendering renders geometry of new views having camera poses . Segmentation Forcing with SAM2 features helps regularize and improve geometry prediction. The scene memory is dynamically updated during reconstruction, while during rendering, the final scene memory output from the reconstruction stage is used without updating. source: [OccAny: Generalized Unconstrained Urban 3D Occupancy](https://arxiv.org/abs/2603.23502)*
