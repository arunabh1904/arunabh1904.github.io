...
_Layer-loop applies each Attention/MoE block repeatedly before advancing through the stack; model-loop traverses the whole stack and then starts again. The local schedule improves parameter reuse and avoids a cyclic pipeline path. Cropped from Figure 1 of the [Loopie paper](https://arxiv.org/abs/2607.16051)._
