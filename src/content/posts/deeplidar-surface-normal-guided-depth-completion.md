...
**Context:** Sparse-to-Dense establishes learned RGB-depth completion; GuideFormer later uses cross-modal transformer guidance for the same runtime contract.

**Takeaway:** Geometry-aware completion improves when the model learns which measurements to distrust, not only how to interpolate the ones marked valid.
