...
For task $i$, the paper evaluates success at epochs $e\in\{0,5,\ldots,50\}$ and selects the best current-task checkpoint. Forward transfer (FWT) averages the current task's learning curve before that best checkpoint, so higher means faster acquisition. Negative backward transfer (NBT) measures the drop on previously seen tasks, so lower means less forgetting. AUC combines current-task learning and retention, so higher is better overall. The metrics are computed from success rate rather than training loss because manipulation loss is a weak proxy for task completion.
