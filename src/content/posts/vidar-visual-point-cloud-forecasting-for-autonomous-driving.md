...
**Context:** UniWorld predicts 4D occupancy; DriveWorld maintains a reusable 4D latent; ViDAR makes future point-cloud forecasting the visual pretraining task.

**Takeaway:** Forecasting geometric evidence forces a visual encoder to model persistence and motion, but the target is privileged supervision—not proof that one future is predictable.
