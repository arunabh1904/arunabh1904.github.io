...
On NAVSIM v1, Table 1 reports 88.4 PDMS for the query-based DriveVLA-W0 expert with one front camera. Multiple trajectory anchors raise the listed result to 90.2, and the autoregressive row reaches 93.0 with the paper’s footnoted best-of-six strategy. The source does not isolate a deployable selection rule for that row, so the result should be read as a best-of-six benchmark figure rather than a guaranteed one-pass score. NAVSIM v2 adds direction, traffic-light, lane-keeping, and extended-comfort terms; Table 2 reports 86.1 EPDMS for its DriveVLA-W0 row. These rows mix decoder strategies, so they show the framework’s range rather than one apples-to-apples comparison.
