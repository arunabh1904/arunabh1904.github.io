...
> DriveVLA-W0 argues that action labels are too sparse to make large driving VLAs use data efficiently. It adds dense future-scene supervision through either autoregressive visual-token prediction or latent diffusion, then attaches a lightweight action expert so the large representation backbone still provides context while avoiding repeated full-backbone action decoding. The experiments connect future-image fidelity, action decoding, and scaling behavior rather than treating them as separate tricks.
...
*Fig 1: The VQ path predicts discrete visual tokens autoregressively, while the ViT path denoises a future-image latent from current visual/action features; both share multimodal context with the action objective. | source: [DriveVLA-W0: World Models Amplify Data Scaling Law in Autonomous Driving, Figure 2](https://arxiv.org/abs/2510.12796)*
...
*Fig 2: The large VLA Expert shares joint attention with a 500M Action Expert; the three panels then compare query-based, autoregressive, and flow-matching action interfaces. | source: [DriveVLA-W0: World Models Amplify Data Scaling Law in Autonomous Driving, Figure 3](https://arxiv.org/abs/2510.12796)*

### Benchmark results and the scaling curve

On NAVSIM v1, Table 1 reports 88.4 PDMS for the query-based DriveVLA-W0 expert with one front camera. Multiple trajectory anchors raise the listed result to 90.2, and the autoregressive row reaches 93.0 with the paper’s footnoted best-of-six strategy. The source does not isolate a deployable selection rule for that row, so the result should be read as a best-of-six benchmark figure rather than a guaranteed one-pass score. NAVSIM v2 adds direction, traffic-light, lane-keeping, and extended-comfort terms; Table 2 reports 86.1 EPDMS for the base configuration. These rows mix decoder strategies, so they show the framework’s range rather than one apples-to-apples comparison.

The main scaling test uses 70 million frames from more than one million clips, alongside 70k- and 700k-frame subsets. These Table 3 rows are deliberately measured on the base VLAs without action experts (Section 4.4), so they isolate the world-model objective from the separate decoder comparison in Table 4. At 70M frames, the VQ action-only baseline has 1.4829 m ADE and 4.88% collision, while VQ plus world modeling reaches 1.0563 m and 3.92%, a 28.8% ADE improvement. The ViT baseline changes from 1.1051 m and 3.59% to 1.0640 m and 3.02%, including a 15.9% collision reduction. Action-only training improves early and then saturates; future-scene supervision keeps extracting signal as the data grow.
...
*Fig 3: The source plot contrasts sparse action supervision with joint visual/action supervision and shows the world-model curve continuing to improve as the in-house frame count grows. | source: [DriveVLA-W0: World Models Amplify Data Scaling Law in Autonomous Driving, Figure 1](https://arxiv.org/abs/2510.12796)*
...
Table 4 reveals a useful reversal. On the 103k-frame NAVSIM training set, the query expert reaches 88.4 PDMS, ahead of flow matching at 87.2 and autoregressive decoding at 85.3. On the 70M-frame in-house set, the autoregressive expert has the lowest ADE and collision—1.0069 m and 2.95%—ahead of flow matching at 1.0362 m and 3.98% and the query expert at 1.1248 m and 4.53%. The authors interpret this reversal as a shift from precision on a simple trajectory distribution to capacity and teacher-forced learning on a more complex one; the rankings alone do not isolate those causes. Flow matching is continuous, yet its sampling and optimization burden becomes a liability at this scale.
